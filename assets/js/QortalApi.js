...
if (typeof qortalRequest === "function") {
  console.log(
    "qortalRequest is available as a function. Setting development mode to false and baseUrl to nothing."
  )
  isOutsideOfUiDevelopment = false
  baseUrl = ""
} else {
  console.log(
    "qortalRequest is not available as a function. Setting baseUrl to localhost."
  )
...
const base64EncodeString = async (str) => {
  const encodedString = btoa(
    String.fromCharCode.apply(
      null,
      new Uint8Array(new TextEncoder().encode(str).buffer)
    )
  )
...
const getAddressInfo = async (address) => {
  ...
  if (!qortalAddressPattern.test(address)) {
    console.warn(
      `Not a valid Qortal address format, returning same thing that was passed to not break other functions: ${address}`
    )
    return address
  }
  try {
...
const fetchOwnerAddressFromName = async (name) => {
  ...
  try {
    const response = await fetch(
      `${baseUrl}/names/${encodeURIComponent(name)}`,
      {
        headers: { Accept: "application/json" },
        method: "GET",
      }
...
const verifyUserIsAdmin = async () => {
  ...
  try {
    ...
    const isAdmin = userGroups.some((group) =>
      adminGroups.includes(group.groupName)
    )
    const isMinterAdmin = minterGroupAdmins.some(
      (admin) => admin.member === userState.accountAddress && admin.isAdmin
    )
    ...
    if (
      ...
      userState.isForumAdmin
    ) {
      console.log(
        `user is one of the following: admin: ${userState.isAdmin} - minterAdmin: ${userState.isMinterAdmin} - forumAdmin: ${userState.isForumAdmin}`
      )
...
const verifyAddressIsAdmin = async (address) => {
  ...
  try {
    ...
    const minterGroupAdmins = await fetchMinterGroupAdmins()
    const isAdmin = await userGroups.some((group) =>
      adminGroups.includes(group.groupName)
    )
    const isMinterAdmin = minterGroupAdmins.some(
      (admin) => admin.member === address && admin.isAdmin
    )
...
const getNameInfo = async (name) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `Failed to fetch name info for: ${name}, status: ${response.status}`
      )
      return null
    }
    ...
    if (!data.name) {
      console.warn(
        `no name info returned, is this not a real registeredName? ${data.name} - returning null to bypass errors...`
      )
      return null
    }
...
const login = async () => {
  try {
    if (
      ...
      (userState.isAdmin || userState.isLoggedIn) &&
    ...
    if (accountNames) {
      ...
      throw new Error(
        "No account names found. Are you logged in? Do you have a registered name?"
      )
    }
  } catch (error) {
...
const fetchMinterGroupAdmins = async () => {
  const response = await fetch(
    `${baseUrl}/groups/members/694?onlyAdmins=true&limit=0&reverse=true`,
    {
      method: "GET",
      headers: { Accept: "application/json" },
    }
  ...
  if (!Array.isArray(admins.members)) {
    throw new Error(
      "Expected 'members' to be an array but got a different structure"
    )
  }
...
const fetchAllAdminGroupsMembers = async () => {
  try {
    ...
    for (const groupID of adminGroupIDs) {
      const response = await fetch(
        `${baseUrl}/groups/members/${groupID}?limit=0`,
        {
          method: "GET",
          headers: { Accept: "application/json" },
        }
...
const fetchAllGroups = async (limit) => {
  ...
  try {
    const response = await fetch(
      `${baseUrl}/groups?limit=${limit}&reverse=true`
    )
    const data = await response.json()
...
const fetchAdminGroupsMembersPublicKeys = async () => {
  try {
    ...
    if (!Array.isArray(adminGroupMemberAddresses)) {
      throw new Error(
        "Expected 'adminGroupMemberAddresses' to be an array but got a different structure"
      )
    }

    if (Array.isArray(adminGroupMemberAddresses)) {
      console.log(
        `adding + minterAdminMemberAddresses:`,
        minterAdminMemberAddresses
      )
      adminGroupMemberAddresses.push(...minterAdminMemberAddresses)
      console.log(
        `final = all adminGroupMemberAddresses`,
        adminGroupMemberAddresses
      )
    }
    ...
    if (!Array.isArray(allMemberPublicKeys)) {
      throw new Error(
        "Expected 'allMemberPublicKeys' to be an array but got a different structure"
      )
    }
...
const fetchGroupInvitesByAddress = async (address) => {
  try {
    const response = await fetch(
      `${baseUrl}/groups/invites/${encodeURIComponent(address)}`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch group invites: HTTP ${response.status}, ${errorText}`
      )
    }
...
const searchLatestDataByIdentifier = async (identifier) => {
  try {
    const response = await fetch(
      `${baseUrl}/arbitrary/resources/search?service=DOCUMENT&identifier=${identifier}&includestatus=true&mode=ALL&limit=0&reverse=true`,
      {
        method: "GET",
        headers: { Accept: "application/json" },
      }
...
const publishMultipleResources = async (
  resources,
  publicKeys = null,
  isPrivate = false
) => {
...
const searchResourcesWithMetadata = async (query, limit) => {
  try {
    ...
    const response = await fetch(
      `${baseUrl}/arbitrary/resources/search?query=${query}&mode=ALL&includestatus=true&includemetadata=true&limit=${limit}&reverse=true`,
      {
        method: "GET",
        headers: { accept: "application/json" },
      }
...
const searchAllResources = async (query, limit, after, reverse = false) => {
  console.log(
    "searchAllResources called. Query:",
    query,
    "Limit:",
    limit,
    "Reverse:",
  ...
  try {
    ...
    const response = await fetch(
      `${baseUrl}/arbitrary/resources/search?query=${query}&mode=ALL&after=${after}&includestatus=false&includemetadata=false&limit=${limit}&reverse=${reverse}`,
      {
        method: "GET",
        headers: { accept: "application/json" },
      }
...
const searchSimple = async (
  ...
  try {
    ...
    if (name && !identifier && !room) {
      ...
      console.error(
        `name: ${name} AND identifier: ${identifier} not passed. Must include at least one...`
      )
      return null
    } else {
      console.log(
        `final searchSimple params = service: '${service}', identifier: '${identifier}', name: '${name}', limit: '${limit}', offset: '${offset}', room: '${room}', reverse: '${reverse}', after: ${after}`
      )
    }

    const response = await fetch(
      `${baseUrl}/arbitrary/resources/searchsimple?${urlSuffix}`,
      {
        method: "GET",
        headers: { accept: "application/json" },
      }
...
const searchAllCountOnly = async (query, room) => {
  try {
    ...
    if (!query.includes(qMintershipForumIdentifierPrefix)) {
      try {
        console.log(
          `'mintership-forum-message' not found, switching to actual query...`
        )
        if (room === "admins") {
          while (hasMore) {
            const response = await qortalRequest({
            ...
            if (response && response.length > 0) {
              ...
              offset = totalCount
              console.log(
                `Fetched ${response.length} items, total count: ${totalCount}, current offset: ${offset}`
              )
            } else {
              hasMore = false
            }
...
const searchResourcesWithStatus = async (query, limit, status = "local") => {
  ...
  try {
    ...
    // Make API request
    const response = await fetch(
      `${baseUrl}/arbitrary/resources/search?query=${query}&includestatus=true&limit=${limit}&reverse=true`,
      {
        method: "GET",
        headers: { accept: "application/json" },
      }
    ...
    if (status) {
      if (status === "notLocal") {
        const notDownloaded = data.filter(
          (resource) => resource.status.status === "published"
        )
        console.log("notDownloaded:", notDownloaded)
        return notDownloaded
      } else if (status === "local") {
        const downloaded = data.filter(
          (resource) =>
...
const getResourceMetadata = async (service, name, identifier) => {
  try {
    const response = await fetch(
      `${baseUrl}/arbitrary/metadata/${service}/${name}/${identifier}`,
      {
        method: "GET",
        headers: { accept: "application/json" },
      }
...
const loadInLineImageHtml = async (
  service,
  name,
  identifier,
  filename,
...
const fetchAndSaveAttachment = async (
  ...
  try {
    ...
    if (service === "FILE_PRIVATE") {
      ...
      if (!response.ok) {
        throw new Error(
          `File not found (HTTP ${response.status}): ${urlPrivate}`
        )
      }
      ...
      if (!response.ok) {
        throw new Error(
          `File not found (HTTP ${response.status}): ${baseUrlWithParams}`
        )
      }
...
const fetchEncryptedImageBase64 = async (
  service,
  name,
  identifier,
  mimeType
  ...
  try {
    ...
    console.error(
      "Skipping file due to error in fetchEncryptedImageBase64:",
      error
    )
...
const renderData = async (service, name, identifier) => {
  ...
  try {
    const response = await fetch(
      `${baseUrl}/render/${service}/${name}?identifier=${identifier}`,
      {
        method: "GET",
        headers: { accept: "*/*" },
      }
...
const processTransaction = async (signedTransaction) => {
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      console.log(
        "Raw text from server (version 2 means JSON string in text):",
        rawText
      )
...
const createGroupInviteTransaction = async (
  recipientAddress,
  adminPublicKey,
  groupId = 694,
  invitee,
  ...
  try {
    ...
    if (!adminPublicKey || !accountReference) {
      throw new Error(
        "Missing required parameters for group invite transaction."
      )
    }
    ...
    if (!response.ok) {
      const errorText = await response.text()
      throw new Error(
        `Failed to create transaction: ${response.status}, ${errorText}`
      )
    }
...
const createGroupKickTransaction = async (
  adminPublicKey,
  groupId = 694,
  member,
...
const createAddGroupAdminTransaction = async (
  ownerPublicKey,
  groupId = 694,
  member,
  txGroupId,
...
const createRemoveGroupAdminTransaction = async (
  ownerPublicKey,
  groupId = 694,
  admin,
  txGroupId,
  ...
  console.log(
    `removeGroupAdminTxCreationInfo:`,
    ownerPublicKey,
    groupId,
    fee,
    txGroupId,
...
const createGroupApprovalTransaction = async (
  adminPublicKey,
  pendingSignature,
  txGroupId = 0,
  fee = 0.01
...
const createGroupBanTransaction = async (
  recipientAddress,
  adminPublicKey,
  groupId = 694,
  offender,
...
const createGroupJoinTransaction = async (
  recipientAddress,
  joinerPublicKey,
  groupId,
  txGroupId = 0,
...
const searchTransactions = async ({
  ...
  try {
    ...
    if (confirmationStatus) {
      queryParams.push(
        `confirmationStatus=${encodeURIComponent(confirmationStatus)}`
      )
    }
    ...
    if (!response.ok) {
      const errorText = await response.text()
      throw new Error(
        `Failed to search transactions: HTTP ${response.status}, ${errorText}`
      )
    }
    ...
    if (!Array.isArray(txArray)) {
      throw new Error(
        "Expected an array of transactions, but got something else."
      )
    }
...
const searchPendingTransactions = async (
  limit = 20,
  offset = 0,
  reverse = false
) => {
  try {
    ...
    const queryString = queryParams.join("&")
    const url = `${baseUrl}/transactions/pending${
    ...
    if (!response.ok) {
      const errorText = await response.text()
      throw new Error(
        `Failed to search pending transactions: HTTP ${response.status}, ${errorText}`
      )
    }
    ...
    if (!Array.isArray(result)) {
      throw new Error(
        "Expected an array for pending transactions, but got something else."
      )
    }
